...
function Lesson({ courseId, userid }) {
    ...
    async function handleUpLoadFile(lessonId, index) {
        ...
        try {
            ...
            // Call createNewMaterial with the uploaded file data
            createNewMaterial(response.data.data, lessonId, index);

            // Reset file selection only after a successful upload
            setFile(null);
        } catch (error) {
            console.error("Upload failed:", error);
            alert("File upload failed.");
    ...
    const lesson_card = lessons.map((lesson, index) => (
        <div key={index} className="p-5 my-3 w-280 mx-auto bg-white border border-gray-400 flex flex-col rounded-xl">
            ...
            <div className="flex flex-col gap-3">
                <div className="ml-3 w-[270px] flex flex-row items-center gap-3">
                    <img className="w-15 h-15" src="https://cdn-icons-png.flaticon.com/512/4662/4662967.png" alt="icon" />
                    <div className="flex flex-col">
                        <h2 className="text-xl font-semibold">Test_C1</h2>
